...
#include "stdafx.h"	/* Definitions of the record structure in the sort file. */
...
int _tmain (int argc, LPTSTR argv [])
{
	...
	if (argc <= iFirstFile) //if no file specified as arg
...
__try {  /* try-except */
	...
	pRecords = (LPBYTE)HeapAlloc (SortHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_NO_SERIALIZE, 
			fsLow + sizeof (TCHAR));
				/* Read the entire file. */

#if defined(UNICODE) || defined(_UNICODE)
	TCHAR bom;//byte order mark known as unicode signature; BOM determines utf encoding and text endianness; 
	ReadFile (hFile, &bom, TSIZE, &nRead, NULL);//read the first character of a file and discard it
	fsLow = fsLow - TSIZE;
#endif
	...
	*(pRecords + fsLow) = _T('\0');
	if (!NoPrint)
		PrintMsg (hStdOut, (LPCTSTR)pRecords);
...
int KeyCompare (LPCVOID pRec1, LPCVOID pRec2)
{
	DWORD i;
	TCHAR b1, b2;
	LPRECORD p1, p2;
