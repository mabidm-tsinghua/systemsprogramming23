...
//           2. lsw.exe -R -l         list the contents of lsw current directory recursively
...
/*           6. D:\systemsprogramming\WSP_4th_edition>.\chapter03\debug\lsw.exe -l ..\..\ 
				...
				c:\* is equivalent to c:\ coz of this program. 
...
int _tmain(int argc, LPCTSTR argv[])
{
	...
	TCHAR searchPattern[MAX_PATH], currPath[MAX_PATH_LONG], parentPath[MAX_PATH_LONG];
	...
	/*
	* Return value of GetCurrentDirectory
	...
	*/
	...
	//argc == fileIndex
	if (argc == fileIndex) //if no arg after options; just this command: lsw -R -l or lsw -l
		...
		//Returns a pointer to the last occurrence of \ in the parentPath string
		...
		if (pSlash != NULL) {
			*pSlash = _T('\0'); 
			...
			pSearchPattern = pSlash + 1;//if c:\ passed as arg then pSlash + 1 is \0
		} else { //no slash in the path; only search pattern specified w/o path
			_tcscpy(parentPath, _T(".\\"));//use current directory as a path
			pSearchPattern = searchPattern;//arg specifies search path
		}
...
/* On entry, the current direcotry is parentPath, which ends in a \ */
{
	...
	if ( _tcslen(searchPattern) == 0 ) {//true if arg c:\ is passed
	...
	for (iPass = 1; iPass <= 2; iPass++) {
		//FindFirstFile lpFileName: The directory or path, and the file name. The file name can include 
		//wildcard characters, for example, an asterisk (*) or a question mark (?).
		//search is w.r.t. Process Current directory which is already set 
		...
		/* For each file/directory located, get the type. List everything on pass 1.
			...
			if (fType == TYPE_DIR && iPass == 2 && recursive) {
				...
				if (_tcslen(parentPath) + _tcslen(findData.cFileName) >= MAX_PATH_LONG) { 
		...
		} while (FindNextFile(searchHandle, &findData));//Continues a file search from a previous call to the FindFirstFile
	...
	the low order part of the file size. */
{
	...
	_tprintf(_T(" %s"), pFileData->cFileName);//complete file/ dir name
	return TRUE;
}
