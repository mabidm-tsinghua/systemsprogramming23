...
int _tmain (int argc, LPTSTR argv[])
{
	...
	TCHAR argstr[MAX_ARG][MAX_COMMAND_LINE];//MAX_ARG: Max num of command line args;  MAX_COMMAND_LINE:Max size of a command line
	...
	while (!exitFlag) {
		_tprintf (_T("%s"), _T("JM$"));
		_fgetts (command, MAX_COMMAND_LINE, stdin);//terminate input with \0
		pc = _tcschr (command, _T('\n'));//return pointer to first occurence of \n
		*pc = _T('\0');

		// breakdown the command into args and store each arg into argstr. pArgc returns total number of args found including program name
		GetArgs (command, (int*)&localArgc, pArgs);
		CharLower (argstr[0]); //Converts a character string or a single character to lowercase
...
int Jobbg (int argc, LPTSTR argv[], LPTSTR command) //argc and argv are similar to main()
{
	...
	LPTSTR targv = SkipArg (command, 1, argc, argv); //skipover jobbg
	...
	if(flags[0] && flags[1])
	   ReportError (_T ("Options -c and -d are mutually exclusive."), 0, FALSE);
	...
	if (argv[1][0] == _T('-'))
		targv = SkipArg (command, 2, argc, argv); //now targv is poiting to command line
	...
	if (!CreateProcess (NULL, targv, NULL, NULL, TRUE,
		...
		/* Create a job number and enter the process Id and process command
			into the Job "data base" maintained by the
			GetJobNumber function (see jobmgt.cpp). */
	_tprintf(_T("jobNumber = GetJobNumber (&processInfo, targv);"));
	jobNumber = GetJobNumber (&processInfo, targv); //-ve job number means 1. max job count reached or 2. database file cannot be opened
	_tprintf(_T("After jobNumber = GetJobNumber (&processInfo, targv) %d;"),jobNumber);
	if (jobNumber >= 0)
		...
		ReportError (_T ("Error: No room in job control list or database cannot be opened."), 0, FALSE);
		return 5;
	}
...
int Kill (int argc, LPTSTR argv[], LPTSTR command)
{
	...
	/* 2nd arg of GenerateConsoleCtrlEvent(): 
