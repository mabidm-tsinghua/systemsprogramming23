	...
	Return -1 in case of failure. */
{
	...
	//file offset of the begining of the lock range
	...
	//lock entire file plus one job record size
	...
	/*	See text comments and Exercise 6-8 regarding a potential defect (and fix)
		...
		//try to find an empty slot (recrod with jobRecord.ProcessId = 0 or process is no more running)  in the database file.
	while (jobNumber < MAX_JOBS_ALLOWED &&
		ReadFile (hJobData, &jobRecord, SJM_JOB, &nXfer, NULL) && (nXfer > 0)) {
		if (jobRecord.ProcessId == 0) break;//empty record ; process is no more running
	...
	//false when database file is empty or file pointer is at the end of a file
	...
	jobRecord.ProcessId = pProcessInfo->dwProcessId;//process id from os
	_tcsnccpy_s (jobRecord.CommandLine, sizeof(jobRecord.CommandLine), Command, MAX_PATH);
	_tprintf(_T("jobRecord Process id = %d\n"),jobRecord.ProcessId);
	_tprintf(_T("jobRecord command line = %s\n"),jobRecord.CommandLine);
	
	if (!WriteFile (hJobData, &jobRecord, SJM_JOB, &nXfer, NULL) || SJM_JOB != nXfer ){
	    ReportError (_T ("Failed to write to DB."), 0, TRUE);
		return -1;
	}
	...
	In the process remove all jobs that no longer exist in the system. */
{
	...
	regionStart.OffsetHigh = 0;
	regionStart.hEvent = (HANDLE)0;
	LockFileEx (hJobData, LOCKFILE_EXCLUSIVE_LOCK, 0, 0, 0, &regionStart);//?lock region size
	...
	while (ReadFile (hJobData, &jobRecord, SJM_JOB, &nXfer, NULL) && (nXfer > 0)) {
		...
		hProcess = OpenProcess (PROCESS_ALL_ACCESS, FALSE, jobRecord.ProcessId);//return NULL if failed.
		if (hProcess != NULL) {
...
/* Create the name of the job management file in the temporary directory. */
{
	...
	if (GetTempPath (TPSize, TempPath) > TPSize || !GetTempPath (TPSize, TempPath)) //temp dir ends with 
	{
		return FALSE;
	}
